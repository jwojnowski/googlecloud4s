...
import me.wojnowski.googlecloud4s.auth.PublicKeyProviderTest.CountAndState
...
class PublicKeyProviderTest extends CatsEffectSuite {
  ...
  test("Cached provider returns the key without a new call") {
    withRecordingKeyProvider {
      case (keyProvider, ref) =>
        for {
          _                 <- ref.update(_.copy(keys = keys))
          cachedKeyProvider <- PublicKeyProvider.cached[IO](keyProvider)
    ...
    for {
      ...
      result <- f(
                  new PublicKeyProvider[IO] {
                    ...
                    override def getAllKeys: IO[Either[PublicKeyProvider.Error, Map[KeyId, Either[PublicKeyProvider.Error, PublicKey]]]] =
                      ref
                        .modify { state =>
                          (
                            state.copy(requestCount = state.requestCount + 1),
                            Right(state.keys.fmap(key => Right(key)))
                          )
                        }
...
object PublicKeyProviderTest {
  private case class CountAndState(requestCount: Int, keys: Map[KeyId, PublicKey])
}
