...
class CollectionListTest extends CatsEffectSuite with FirestoreTestContainer {
  ...
  test("no collections to list (root level)") {
    withFirestore { implicit firestore =>
      for {
        collectionIds <- (firestore / projectId / databaseId).listCollectionIds().compile.toList
  ...
  test("no collections to list (document level)") {
    withFirestore { implicit firestore =>
      for {
        ...
        collectionIds <- (collectionA / documentAName).listCollectionIds().compile.toList
      } yield assertEquals(collectionIds, List.empty)
    }
  }
  ...
  test("multiple pages of collections") {
    withFirestore { implicit firestore =>
      for {
        _             <- createCollections
        collectionIds <- (firestore / projectId / databaseId).listCollectionIds(pageSize = Some(2)).compile.toList
      } yield assertEquals(collectionIds, rootLevelCollections.map(_.collectionId))
    }
  }
  ...
  test("subcollections") {
    withFirestore { implicit firestore =>
      for {
        _             <- createCollections
        collectionIds <- collectionAA.parent.listCollectionIds().compile.toList
